    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        /*switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                downX = event.getX();
                downY = event.getY();
                //每次开始按下,横向滑动为否, 忽略滑动为否
                horizonMove = false;
                ignoreMove = false;
                return true;
            case MotionEvent.ACTION_MOVE:
                if (ignoreMove) {
                    Log.i("movetest", "ignoreMove");
                    break;
                }
                //滑动到的xy
                double x = event.getX();
                ...
                if (!horizonMove) {
                    if ((dx / dy) > 2 || (dy / dx) > 2) {
                        Log.i("movetest", "get horizonMove");
                        horizonMove = true;
                    } else {
                        Log.i("movetest", "get vertialMove then ignoreMove");
                        ignoreMove = true;
                    }
                } else {
                    //处理横向滑动
                    //消费掉事件
                    Log.i("movetest", "handle move");
                }
                break;
            default:
                break;
