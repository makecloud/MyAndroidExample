    ...
    public static boolean runAsRoot(String cmd) {
        try {
            ...
            if (result.result == 0) {
                ...
                Toast.makeText(MyApplication.getContextObject(), "error Msg: " + result.errorMsg, Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
            Toast.makeText(MyApplication.getContextObject(), e.getMessage(), Toast.LENGTH_SHORT).show();
        }
        return false;
    }
